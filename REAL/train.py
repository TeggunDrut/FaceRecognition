...
model = tf.keras.Sequential(
    [
        ...
        tf.keras.layers.Conv2D(
            32, (3, 3), activation="relu", input_shape=(240, 320, 3)
        ),
        tf.keras.layers.MaxPooling2D((2, 2)),
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(64, activation="relu"),
        tf.keras.layers.Dense(4, activation="linear"),  # 4 outputs for (x, y, w, h)
    ]
)
...
labels[:, [0, 2]] /= 320  # Normalize x and w
labels[:, [1, 3]] /= 240  # Normalize y and h

# Train the model
model.fit(images, labels, epochs=10)  # You can adjust the number of epochs
